...
class TestObservableList(ObservableTestCase]:
    ...
    def test_list_copy_behavior(self]:
        """Test that value returns immutable tuple"""
        obs = ObservableList([1, 2, 3]]
        
        # Get the tuple value
        tuple_value = obs.value
        
        # Verify it's a tuple (immutable]
        assert isinstance(tuple_value, tuple]
        assert tuple_value == [1, 2, 3]
        
        # Tuple cannot be modified
        try:
            tuple_value.append(4]  # type: ignore
    ...
    def test_serialization(self]:
        ...
        assert serialized_data["value"] == expected_list
