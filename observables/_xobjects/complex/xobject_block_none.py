...
class ObservableBlockNone(CarriesSomeHooksBase[Literal["value_without_none", "value_with_none"], T, "ObservableBlockNone[T]"], Generic[T]):
    """
    ...
    def __init__(
        ...
        if hook_with_None is not None:
            self._hook_with_None.join(self._hook_without_None, "use_target_value") # type: ignore
        if hook_without_None_or_value is not None:
            self._hook_without_None.join(self._hook_with_None, "use_target_value") # type: ignore
